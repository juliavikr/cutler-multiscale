...
#   PSEUDO_LABEL_NAME=combined5   sbatch slurm/run_training_luiz.sh
#
# Expected files:
...
#   ${DATA_ROOT}/tiny-imagenet-5/annotations/v1_combined_pseudo.json
...
case "${PSEUDO_LABEL_NAME}" in
    ...
    final5)
        ...
        ;;
    combined5)
        PSEUDO_JSON="${ANNO_DIR_5C}/v1_combined_pseudo.json"
        DATASET_NAME="tinyimagenet_5c_combined_pseudo"
        ;;
    *)
        echo "ERROR: Unknown PSEUDO_LABEL_NAME '${PSEUDO_LABEL_NAME}'. Expected 'baseline5', 'multiscale5', 'final5', or 'combined5'."
        exit 1
        ;;
esac
