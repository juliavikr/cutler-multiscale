...
def maskcut_multicrop(
    ...
    I = Image.open(img_path).convert("RGB")
    I_fixed = I.resize((int(fixed_size), int(fixed_size)), PIL.Image.LANCZOS)
    ...
    for x, y, w, h in windows:
        crop = I_fixed.crop((x, y, x + w, y + h))
        bipartitions, _, crop_resized = maskcut_from_pil(
            crop, backbone, patch_size, tau, N=N, fixed_size=w, cpu=cpu
        )
        for bipartition in bipartitions:
            refined = postprocess_crop_mask(crop_resized, bipartition)
            if refined is None:
                continue
            full_mask = np.zeros((fixed_size, fixed_size), dtype=np.bool_)
            full_mask[y:y + h, x:x + w] = np.logical_or(full_mask[y:y + h, x:x + w], refined)
