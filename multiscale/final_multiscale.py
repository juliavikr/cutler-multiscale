...
# Final multiscale variant:
# baseline-first hybrid pseudo labels. Full-image MaskCut masks are treated as
# trusted anchors; crop masks are only conservative rescue additions after
# fragment merging, duplicate suppression, and strict shape/quality filtering.
...
def fraction_inside(mask_a, mask_b):
    a = mask_a.astype(np.bool_)
    b = mask_b.astype(np.bool_)
    area = float(a.sum())
    if area <= 0:
        return 0.0
    return float(np.logical_and(a, b).sum()) / area


def union_valid_for_final(
    union,
    max_area_ratio,
    max_aspect_ratio,
    min_compactness,
    ...
    if union is None or not union.any():
        return False
    if mask_area_ratio(union) > max_area_ratio:
        return False
    if mask_aspect_ratio(union) > max_aspect_ratio:
        return False
    if mask_compactness(union) < min_compactness:
        return False
    return True


def passes_final_crop_quality(
    candidate,
    min_area_ratio,
    max_area_ratio,
    min_score,
    min_compactness,
    max_aspect_ratio,
    max_internal_border_touch,
    max_internal_border_sides,
    max_crop_fill,
    min_crf_iou,
):
    meta = candidate.meta
    ...
    if area_ratio < min_area_ratio or area_ratio > max_area_ratio:
        return False
    if candidate.mask_score < min_score:
        return False
    if meta.get("compactness", mask_compactness(candidate.mask)) < min_compactness:
        return False
    if meta.get("aspect_ratio", mask_aspect_ratio(candidate.mask)) > max_aspect_ratio:
        return False
    if meta.get("internal_border_touch", 0.0) > max_internal_border_touch:
        return False
    if meta.get("internal_border_sides", 0) > max_internal_border_sides:
        return False
    if meta.get("crop_fill", 0.0) > max_crop_fill:
        return False
    if meta.get("crf_iou", 1.0) < min_crf_iou:
        return False
    return True


def merge_final_crop_fragments(
    crop_candidates,
    min_area_ratio,
    max_area_ratio,
    merge_iou_thresh,
    ...
    candidates = list(crop_candidates or [])
    if not candidates:
        return []

    n = len(candidates)
    boxes = [mask_bbox(c.mask) for c in candidates]
    adj = [set() for _ in range(n)]
    for i in range(n):
        for j in range(i + 1, n):
            connected = binary_iou(candidates[i].mask, candidates[j].mask) > merge_iou_thresh
            if not connected:
                connected = intersection_over_smaller(candidates[i].mask, candidates[j].mask) > containment_thresh
            if not connected and boxes[i] is not None and boxes[j] is not None:
                connected = boxes_overlap_expanded(boxes[i], boxes[j], box_expand_ratio)
            if connected:
                adj[i].add(j)
    ...
    for start in range(n):
        ...
        if union_valid_for_final(
            ...
            best_member = max(members, key=lambda c: c.mask_score)
            merged_candidate = make_mask_candidate(
                union,
                source="final_fragment_merge",
                crop_box=best_member.crop_box,
                crop_score=best_member.crop_score,
                crop_reason=best_member.meta.get("crop_reason"),
                protected_masks=None,
            ...
            merged_candidate.mask_score = max(c.mask_score for c in members) - 0.04 * (len(members) - 1)
            merged_candidate.meta["mask_score"] = float(merged_candidate.mask_score)
            merged_candidate.meta["component_size"] = int(len(members))
            merged_candidate.meta["component_sources"] = [c.source for c in members]
            merged.append(merged_candidate)
        else:
            if stats is not None:
                stats["final_rejected_union"] += max(0, len(members) - 1)
            merged.append(max(members, key=lambda c: c.mask_score))

    merged.sort(key=lambda c: (-c.mask_score, mask_area_ratio(c.mask)))
    return merged


def conservative_final_merge(
    ...
    box_expand_ratio,
    max_aspect_ratio,
    max_new_crop_masks=2,
    min_score=1.0,
    min_compactness=0.22,
    min_merged_compactness=0.18,
    max_internal_border_touch=0.55,
    max_internal_border_sides=1,
    max_crop_fill=0.90,
    min_crf_iou=0.50,
    duplicate_iou_thresh=0.35,
    duplicate_inside_thresh=0.70,
    stats=None,
):
    protected = list(protected_candidates or [])
    quality_kept = []
    for candidate in crop_candidates or []:
        if passes_final_crop_quality(
            candidate,
            ...
            min_compactness=min_compactness,
            max_aspect_ratio=max_aspect_ratio,
            max_internal_border_touch=max_internal_border_touch,
            max_internal_border_sides=max_internal_border_sides,
            max_crop_fill=max_crop_fill,
            min_crf_iou=min_crf_iou,
        ):
            quality_kept.append(candidate)
        elif stats is not None:
            stats["final_rejected_quality"] += 1

    crop_rescues = merge_final_crop_fragments(
        quality_kept,
        min_area_ratio=min_area_ratio,
        max_area_ratio=max_area_ratio,
        ...
        max_aspect_ratio=max_aspect_ratio,
        min_compactness=min_merged_compactness,
        stats=stats,
    )

    final = list(protected)
    added = []
    for candidate in crop_rescues:
        duplicate = False
        fragment = False
        for protected_candidate in protected:
            if binary_iou(candidate.mask, protected_candidate.mask) >= duplicate_iou_thresh:
                duplicate = True
                break
            if fraction_inside(candidate.mask, protected_candidate.mask) >= duplicate_inside_thresh:
                fragment = True
                break
            if intersection_over_smaller(candidate.mask, protected_candidate.mask) >= containment_thresh:
                fragment = True
                break
        if duplicate:
            if stats is not None:
                stats["final_rejected_duplicate"] += 1
            continue
        if fragment:
            if stats is not None:
                stats["final_rejected_fragment"] += 1
            continue

        if any(binary_iou(candidate.mask, kept.mask) >= duplicate_iou_thresh for kept in added):
            if stats is not None:
                stats["final_rejected_duplicate"] += 1
            continue
        if any(intersection_over_smaller(candidate.mask, kept.mask) >= containment_thresh for kept in added):
            if stats is not None:
                stats["final_rejected_duplicate"] += 1
            continue
        ...
        if max_new_crop_masks > 0 and len(added) >= max_new_crop_masks:
            break

    if stats is not None:
        stats["final_masks"] = len(final)
        stats["final_added_masks"] = len(added)
    return final
...
def maskcut_multicrop(
    ...
    crop_fill_thresh=0.9,
    crf_iou_thresh=0.3,
    final_max_new_crop_masks=2,
    final_min_score=1.0,
    final_min_compactness=0.22,
    final_min_merged_compactness=0.18,
    final_max_internal_border_touch=0.55,
    final_max_internal_border_sides=1,
    final_duplicate_iou_thresh=0.35,
    final_duplicate_inside_thresh=0.70,
    return_stats=False,
    return_splits=False,
    return_debug=False,
):
    ...
    stats = {
        ...
        "final_masks": 0,
        "final_added_masks": 0,
        "final_rejected_quality": 0,
        "final_rejected_duplicate": 0,
        "final_rejected_fragment": 0,
        "final_rejected_union": 0,
        "scored_candidates": 0,
    ...
    final_candidates = conservative_final_merge(
        protected_candidates=protected_masks,
        ...
        box_expand_ratio=box_expand_ratio,
        max_aspect_ratio=merge_max_aspect_ratio,
        max_new_crop_masks=final_max_new_crop_masks,
        min_score=final_min_score,
        min_compactness=final_min_compactness,
        min_merged_compactness=final_min_merged_compactness,
        max_internal_border_touch=final_max_internal_border_touch,
        max_internal_border_sides=final_max_internal_border_sides,
        max_crop_fill=crop_fill_thresh,
        min_crf_iou=crf_iou_thresh,
        duplicate_iou_thresh=final_duplicate_iou_thresh,
        duplicate_inside_thresh=final_duplicate_inside_thresh,
        stats=stats,
    )
    ...
    if return_splits:
        if return_debug:
            if return_stats:
                return splits["final"], I, stats, splits, debug_splits
            return splits["final"], I, splits, debug_splits
        if return_stats:
            return splits["final"], I, stats, splits
        return splits["final"], I, splits
    if return_stats:
        return splits["final"], I, stats
    return splits["final"], I
...
if __name__ == "__main__":
    ...
    parser.add_argument('--final-max-new-crop-masks', type=int, default=2, help='final variant: max high-confidence crop rescue masks added after baseline masks')
    parser.add_argument('--final-min-score', type=float, default=1.0, help='final variant: minimum crop candidate quality score before rescue')
    parser.add_argument('--final-min-compactness', type=float, default=0.22, help='final variant: minimum compactness for crop candidates')
    parser.add_argument('--final-min-merged-compactness', type=float, default=0.18, help='final variant: minimum compactness for merged crop fragments')
    parser.add_argument('--final-max-internal-border-touch', type=float, default=0.55, help='final variant: reject crop masks that strongly touch internal crop borders')
    parser.add_argument('--final-max-internal-border-sides', type=int, default=1, help='final variant: reject crop masks touching more than this many internal crop sides')
    parser.add_argument('--final-duplicate-iou-thresh', type=float, default=0.35, help='final variant: suppress crop masks with this IoU to baseline/final masks')
    parser.add_argument('--final-duplicate-inside-thresh', type=float, default=0.70, help='final variant: suppress crop masks mostly contained in baseline masks')
    parser.add_argument('--primary-output', type=str, default='final', choices=['normal', 'raw_multiscale', 'multiscale', 'combined', 'final'], help='which split is written to the unsuffixed JSON/checkpoint in multi-crop mode')
            ...
            try:
                if args.multi_crop:
                    bipartitions, I_new, multicrop_stats, split_masks, split_debug = maskcut_multicrop(
                        ...
                        final_max_new_crop_masks=args.final_max_new_crop_masks,
                        final_min_score=args.final_min_score,
                        final_min_compactness=args.final_min_compactness,
                        final_min_merged_compactness=args.final_min_merged_compactness,
                        final_max_internal_border_touch=args.final_max_internal_border_touch,
                        final_max_internal_border_sides=args.final_max_internal_border_sides,
                        final_duplicate_iou_thresh=args.final_duplicate_iou_thresh,
                        final_duplicate_inside_thresh=args.final_duplicate_inside_thresh,
                        return_stats=True,
                        return_splits=True,
                        return_debug=True,
                    )
            ...
            if args.multi_crop:
                ...
                if args.log_every > 0 and processed_images % args.log_every == 0:
                    print(
                        ...
                        "windows={} skipped={} ranked={} rescue={} retries={} generated={} rejected={} crop_candidates={} scored={} crop_merged={} merged={} final={} added={} fqrej={} fdupe={} ffrag={} funion={}".format(
                            processed_images,
                            ...
                            multicrop_totals["final_masks"],
                            multicrop_totals["final_added_masks"],
                            multicrop_totals["final_rejected_quality"],
                            multicrop_totals["final_rejected_duplicate"],
                            multicrop_totals["final_rejected_fragment"],
                            multicrop_totals["final_rejected_union"],
                        )
                    )
    ...
    if args.multi_crop and processed_images > 0:
        print(
            ...
            "skipped={} ranked={} rescue={} retries={} generated={} rejected={} crop_candidates={} scored={} crop_merged={} merged={} final={} added={} fqrej={} fdupe={} ffrag={} funion={}".format(
                processed_images,
                multicrop_totals["full_masks"],
                multicrop_totals["total_windows"],
                multicrop_totals["skipped_covered"],
